  ...
  return fetch(url, {
    headers: new Headers({
      Authorization:
        "Basic " +
        base64.encode(
          authData.basic_auth.username + ":" + authData.basic_auth.password
        ),
    }),
  })
    .then((response) => {
      switch (response.status) {
        ...
        case 500:
          throw new Error(
            "Internal Server Error. The server failed to fulfill the request"
          );

        default:
          // Return generic error message if response status were not included in cases
          throw new Error(
            "The service responded with status code: " + response.status
          );
      }
    })
    .catch((error) => {
